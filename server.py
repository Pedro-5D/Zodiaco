...
def calculate_positions_aspects(positions):
    ...
    aspects = []
    # Lista completa de planetas tradicionales
    ...
    # Filtrar correctamente los planetas tradicionales
    planet_positions = [p for p in positions if p["name"] in traditional_planets]
    
    # Imprimir información de debug para verificar qué planetas se están procesando
    print(f"Planetas tradicionales encontrados: {[p['name'] for p in planet_positions]}")
    ...
    # Imprimir información de debug para verificar los aspectos calculados
    print(f"Total de aspectos calculados: {len(aspects)}")
    print(f"Aspectos por planeta: {[(p, len([a for a in aspects if a['planet1'] == p or a['planet2'] == p])) for p in traditional_planets]}")
    
    return aspects

def calculate_positions_aspects_between_sets(positions1, positions2):
    """Calcula aspectos entre dos conjuntos de posiciones planetarias (natal y tránsitos)"""
    aspects = []
    # Lista completa de planetas tradicionales
    traditional_planets = ["SOL", "LUNA", "MERCURIO", "VENUS", "MARTE", "JÚPITER", "SATURNO"]
    ...
    # Filtrar correctamente los planetas tradicionales de ambos conjuntos
    planets1 = [p for p in positions1 if p["name"] in traditional_planets]
    planets2 = [p for p in positions2 if p["name"] in traditional_planets]
    
    # Imprimir información de debug para verificar qué planetas se están procesando
    print(f"Planetas natales encontrados: {[p['name'] for p in planets1]}")
    print(f"Planetas de tránsito encontrados: {[p['name'] for p in planets2]}")
    
    # Aspectos entre planetas de diferentes conjuntos
    for pos1 in planets1:
        pos1["chart_type"] = "natal"  # Marcar como natal
        for pos2 in planets2:
            pos2["chart_type"] = "transit"  # Marcar como tránsito
            
            # Saltar si es el mismo planeta (lo manejamos más abajo)
    ...
    # Aspectos entre el mismo planeta en ambos conjuntos (natal vs tránsito)
    for pos1 in planets1:
        pos1["chart_type"] = "natal"  # Marcar como natal
        for pos2 in planets2:
            pos2["chart_type"] = "transit"  # Marcar como tránsito
            
            # Solo procesar cuando es el mismo planeta
            if pos1["name"] == pos2["name"]:
                angle = calculate_angle(pos1["longitude"], pos2["longitude"])
                aspect_type = determine_aspect_type(angle)
    ...
    # Imprimir información de debug para verificar los aspectos calculados
    print(f"Total de aspectos entre cartas calculados: {len(aspects)}")
    for p in traditional_planets:
        natal_aspects = len([a for a in aspects if a['planet1'] == p])
        transit_aspects = len([a for a in aspects if a['planet2'] == p])
        print(f"Planeta {p}: {natal_aspects} aspectos como natal, {transit_aspects} aspectos como tránsito")
